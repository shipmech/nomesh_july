...
class MeshNodeDictData():
    def __init__(self, device, node_pos):
        ...
        # Не меняется во время симуляции
        self.dict_type_index_to_type_name = {
            0 : 'Free',
            1 : 'Press',
            2 : 'NoSlip'
        }
        ...
        for node_index in range(len(self.nodes)):
            self.dict_node_index_to_enabled_BCs[node_index] = to_torch_int([0,0], self.device)
    ...
    def add_BC(self, bc : BoundaryCondition, geometry_selection_cond):
        ...
        self.dict_BC_index_to_BCType_index[bc_index] = self.dict_BCType_name_to_BC_type_index[self.dict_BCType_name_to_BC_type_index[bc.type_name]]
        ...
        for node_index in range(len(self.nodes)):
            for node_type_index, bc_type_tensor in self.dict_type_index_to_enabled_BCs.items():
                if torch.allclose(self.dict_node_index_to_enabled_BCs[node_index], bc_type_tensor):
                    self.dict_node_index_to_type_index[node_index] = node_type_index
...
class GraphMesh:
    ...
    def generate_graph_data(self):
        ...
        num_base_f = self.config.number_of_free_latent_features
        ...
        dict_node_to_type_index = self.node_dict_data.dict_node_index_to_type_index
        ...
        for i_edge in range(edge_index.shape[1]):
            src_type_index = dict_node_to_type_index[edge_index[0][i_edge]]
            dst_type_index = dict_node_to_type_index[edge_index[1][i_edge]]
...
class BackgroundMesh(GraphMesh):
    ...
    def initialize_mesh(self, nodes_positions : np.array, bc_list : list):
        nodes, edges, faces = TriangleMeshGenerator(nodes_positions)
        self.pos = to_torch_float(nodes, self.device)
        self.edge_index = to_torch_float(edges, self.device)
        self.faces = to_torch_float(faces, self.device)
